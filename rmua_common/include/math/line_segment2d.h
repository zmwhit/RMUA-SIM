...
public:
  ...
  double DistanceToRay(const Vec2d &ray_origin, double ray_direction);
  ...
  inline std::vector<Vec2d> SamplePoints(int num) {
    return SamplePoints(length_ / (num - 1));
  }

  std::vector<Vec2d> SamplePoints(double step);
