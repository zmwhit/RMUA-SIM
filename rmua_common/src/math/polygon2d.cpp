...
std::pair<int, double> Polygon2d::DistanceToPoint(const Vec2d &point) const {
  if (IsPointIn(point)) {
    return {0, 0.0};
  }
  double distance = std::numeric_limits<double>::infinity();
  int index = 0;
  for (int i = 0; i < num_points_; ++i) {
    double dis = line_segments_[i].DistanceTo(point);
    if (dis < distance) {
      distance = dis;
      index = i;
    }
...
Polygon2d Polygon2d::ExpandEdgeByDistance(const double exp_dis, const double sep_dis, const double kMinAngle) const {
  if (!is_convex_) {
    Polygon2d convex_polygon;
    ComputeConvexHull(points_, &convex_polygon);
    return convex_polygon.ExpandEdgeByDistance(exp_dis, sep_dis, kMinAngle);
  }
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    if (diff <= kMathEpsilon) {
      points.push_back(points_[i] +
                       Vec2d::CreateUnitVec2d(start_angle) * exp_dis);
    } else {
      const int count = static_cast<int>(diff / kMinAngle) + 1;
      for (int k = 0; k <= count; ++k) {
        const double angle = start_angle + diff * static_cast<double>(k) /
                                           static_cast<double>(count);
        points.push_back(points_[i] + Vec2d::CreateUnitVec2d(angle) * exp_dis);
      }
    }
  }
  Polygon2d new_polygon;
  ComputeConvexHull(points, &new_polygon);

  double kMinDis = sep_dis;
  std::vector<Vec2d> final_points;
  auto new_points = new_polygon.points();
  new_points.emplace_back(new_points.front());
  for (int i = 0; i < new_points.size() - 1; ++i) {
      int num = new_points[i].DistanceTo(new_points[i+1])/kMinDis;
      if (num == 0) {
        final_points.emplace_back(new_points[i]);
      } else {
        for(int j = 0; j < num; ++j) {
            final_points.emplace_back(new_points[i].x() + (double)j/num*(new_points[i+1].x() - new_points[i].x()),
                                      new_points[i].y() + (double)j/num*(new_points[i+1].y() - new_points[i].y()));
        }        
      }

  }

  new_polygon = Polygon2d(final_points);
  return new_polygon;
}
